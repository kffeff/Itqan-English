...
# إخفاء النص الزائد على زر الإعدادات
components.html("""
<script>
function hidePopoverText() {
    var btns = window.parent.document.querySelectorAll('[data-testid="stPopover"] button');
    btns.forEach(function(btn) {
        btn.childNodes.forEach(function(node) {
            if (node.nodeType === 3 && node.textContent.trim() !== '') {
                node.textContent = '';
            }
            if (node.tagName === 'SPAN' && !node.querySelector('svg')) {
                node.style.display = 'none';
            }
        });
    });
}
setTimeout(hidePopoverText, 500);
setTimeout(hidePopoverText, 1500);
</script>
""", height=0)
